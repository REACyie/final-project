首先确定ansible脚本的整体结构（参考了chatgpt4.0），主配置文件（参考了chatgpt4.0）ansible.cfg，再创建三个目录分别为安装配置apache服务器脚本，初始化更改mysql数据库的数据，以及初始化更改mysql数据库的脚本，php网页是参考的网上的模板进行的修改以及chatgpt4.0的帮助进行的修改，修改hosts文件中的ip地址就实现自动化配置。
